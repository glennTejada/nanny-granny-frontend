            ...
            <template slot="header">
                <v-layout row align-center layout--header>
                    ...
                    <v-flex>
                        ...
                        <p class="text-small mt-3">Member since {{nanny.created_at | formatDate('MMM YYYY')}}</p>
                    </v-flex>
                </v-layout>
            </template>
            <template slot="about">
...
<script>
  ...
  import AppWindowImageGallery from '../app-windows/app-window-image-gallery'

  export default {
    name: 'app-dialog-nanny-profile',

    components: {AppBrandshapeBorder, AppWindowImageGallery, AppAvatar, AppLayoutProfile},

    props: ['rawData'],

    data () {
      return {
        nanny: this.rawData.profile,
        gallery: false
      }
    },
    computed: {
